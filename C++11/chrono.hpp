...
//the running time may vary to your machine's current available resources
	...
	case T_FORMAT::F_ALL :
		...
		// using C++14's literals for time
		using namespace std::chrono_literals;
		minutes mm = duration_cast<minutes>(dur % 1h);
		seconds ss = duration_cast<seconds>(dur % 1min);
		milliseconds ms = duration_cast<milliseconds>(dur % 1s);
		microseconds us = duration_cast<microseconds>(dur % 1ms);
